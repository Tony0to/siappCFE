...
class Module3Tema2Screen extends StatefulWidget {
  ...
  @override
  State<Module3Tema2Screen> createState() => _Module3Tema2ScreenState();
}

class _Module3Tema2ScreenState extends State<Module3Tema2Screen> with TickerProviderStateMixin {
  ...
  static final Map<String, String> _sectionImages = {
    ...
    'algoritmos_busqueda': 'https://images.unsplash.com/photo-1517694712202-14dd9538aa97?ixlib=rb-4.0.3&auto=format&fit=crop&w=1470&q=80', // Imagen para portada
    'busqueda_binaria': 'https://images.unsplash.com/photo-1635070041078-e363dbe005cb?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1470&q=80', // Diagrama de división binaria
  ...
  Future<void> _loadJsonContent() async {
    ...
    try {
      ...
      final sectionContent = data['content']['section_2'];

      final activities = data['activities'] as List<dynamic>;
      final tema2Activity = activities.firstWhere(
        (activity) => activity['subtopic'] == 'Algoritmos de búsqueda y ordenamiento',
        orElse: () => null,
      );

      if (tema2Activity == null) {
        throw Exception('No se encontró la actividad para el tema 2');
      }

      setState(() {
        _contentData = {
          ...sectionContent,
          'objective': tema2Activity['objective'],
          'theory': tema2Activity['theory'],
          'reflection': tema2Activity['reflection'],
          'practice': tema2Activity['practice'],
        };
        _quizQuestions = tema2Activity['theory']['questions'];
  ...
  Widget _buildSectionImage() {
    final imageUrl = _sectionImages['algoritmos_busqueda']!;
    return Container(
      height: 220,
            ...
            child: Column(
              ...
              children: [
                ConstrainedBox(
                  ...
                  child: Text(
                    _contentData?['title']?.toString() ?? widget.section['title'] ?? 'Algoritmos de búsqueda',
  ...
  Map<String, String>? _getImageForSubsection(String title) {
    ...
    if (lowerTitle.contains('binaria') || lowerTitle.contains('mitades') || lowerTitle.contains('o(log n)')) {
      debugPrint('Asignando imagen: busqueda_binaria');
      return {'key': 'busqueda_binaria', 'caption': 'Búsqueda binaria'};
    }
    if (lowerTitle.contains('burbuja') || lowerTitle.contains('inserción') || lowerTitle.contains('insercion') || lowerTitle.contains('selección') || lowerTitle.contains('seleccion')) {
      debugPrint('Asignando imagen: ordenamiento_basico');
      return {'key': 'ordenamiento_basico', 'caption': 'Ordenamiento básico'};
    }
    if (lowerTitle.contains('merge sort') || lowerTitle.contains('quicksort') || lowerTitle.contains('o(n log n)')) {
  ...
  void _navigateNext() {
    ...
    if (widget.sectionIndex + 1 < widget.totalSections) {
      ...
      Navigator.pushReplacement(
        context,
        PageRouteBuilder(
          pageBuilder: (context, animation, secondaryAnimation) => Module3Tema3Screen(
            section: nextSection,
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: Text(
            widget.section['title'] ?? 'Algoritmos de búsqueda',
