...
class InventorySystemActivityScreen extends StatefulWidget {
  const InventorySystemActivityScreen({super.key});
...
class _InventorySystemActivityScreenState extends State<InventorySystemActivityScreen> {
  ...
  List<String> _userOrder = List.filled(19, '');
  ...
  void _initializeActivity() {
    final random = Random();
    final indices = List<int>.generate(19, (i) => i)..shuffle(random);

    final prefilledIndices = indices.sublist(0, 14); // 50% pre-filled
    _userOrder = List.filled(19, '');

    for (final i in prefilledIndices) {
      _userOrder[i] = _correctOrder[i];
    }

    _availableLines = indices.sublist(10).map((i) => _correctOrder[i]).toList();
    _initialUserOrder = List.from(_userOrder);
    setState(() {});
  }

  int _getIndentationLevel(String line) {
    if (line.contains('inventario[nombre] ← precio')) {
      return 1;
    } else if (line.contains('PARA cada producto en inventario HACER') ||
        line.contains('IMPRIMIR producto, ":", inventario[producto]') ||
        line.contains('FIN PARA')) {
      return 1;
    } else if (line.contains('total ← 0') ||
        line.contains('PARA cada precio en inventario HACER') ||
        line.contains('total ← total + precio') ||
        line.contains('IMPRIMIR "Valor total del inventario:", total')) {
      return 1;
    }
    return 0;
  }
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              Text(
                '• Cada línea correcta aporta 5.26% a la calificación total.',
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                GlassmorphicCard(
                  child: Text(
                    'Ordena las líneas de código para completar el algoritmo de sistema de inventario. Arrastra las líneas disponibles a las posiciones correctas.',
                ...
                GlassmorphicCard(
                  child: Column(
                    ...
                    children: [
                      Text(
                        'Algoritmo de Sistema de Inventario:',
