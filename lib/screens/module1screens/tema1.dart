...
class _Tema1ScreenState extends State<Tema1Screen> with TickerProviderStateMixin {
  ...
  Future<void> _loadJsonContent() async {
    ...
    try {
      ...
      // Obtener el contenido de la sección 1
      final sectionContent = data['content']['section_1'];

      // Buscar la actividad correspondiente al tema 1 (Conceptos clave antes de escribir código)
      final activities = data['activities'] as List<dynamic>;
      final tema1Activity = activities.firstWhere(
        (activity) => activity['subtopic'] == 'Conceptos clave antes de escribir código',
        orElse: () => null,
      );
  ...
  Widget _buildSectionImage() {
    final imageUrl = _sectionImages['lenguaje_programacion']!;
    return Container(
      height: 220,
            ...
            child: Column(
              ...
              children: [
                ConstrainedBox(
                  ...
                  child: Text(
                    _contentData?['title']?.toString() ?? widget.section['title'] ?? 'Conceptos clave antes de escribir código',
  ...
  void _navigateNext() {
    ...
    if (widget.sectionIndex + 1 < widget.totalSections) {
      ...
      Navigator.pushReplacement(
        context,
        PageRouteBuilder(
          pageBuilder: (context, animation, secondaryAnimation) => Tema2Screen(
            section: nextSection,
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: Text(
            widget.section['title'] ?? 'Conceptos clave antes de escribir código',
