...
class _Tema1State extends State<Tema1> with TickerProviderStateMixin {
  ...
  Future<void> _loadJsonContent() async {
    try {
      ...
      final sectionData = data['content']['section_1'] as Map<String, dynamic>?;
      if (sectionData == null) {
        throw Exception('No se encontró section_1 en el JSON');
      }
      setState(() {
        _contentData = {
  ...
  void _initializeVideo() {
    setState(() {
      _showVideo = true;
      try {
        final videoId = _contentData?['video']?['videoId']?.toString() ?? 'ImOR0o-QHOQ';
  ...
  Widget buildDiagramImage(String? imagePath, {double height = 350, bool enableZoom = true}) {
        ...
        child: Column(
          ...
          children: [
            ...
            if (example['image'] != null && example['image'].isNotEmpty) ...[
              const SizedBox(height: 10),
              buildDiagramImage(example['image']?.toString(), height: 200, enableZoom: true),
            ],
            const SizedBox(height: 10),
  ...
  Widget buildSubsectionPage(Map<String, dynamic>? sectionData, int index, int totalPages) {
    ...
    final activity = _activities.firstWhere(
      (act) => act['subtopic'] == 'Conceptos clave antes de escribir código',
      orElse: () => {},
    );
